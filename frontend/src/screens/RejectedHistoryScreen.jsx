...
const RejectedHistoryScreen = ({ navigation }) => {
  ...
  const fetchRequests = async (silent = false) => {
    try {
      ...
      const sorted = res.data.filter(r => r.status === 'Rejected');
  ...
  const HistoryCard = ({ item }) => {
    return (
        <View style={styles.card}>
            <View style={[styles.cardAccent, { backgroundColor: '#e11d48' }]} />
            <View style={styles.cardHeader}>
                ...
                <View style={[styles.statusBadge, { backgroundColor: '#fff1f2' }]}>
                    <Text allowFontScaling={false} style={[styles.badgeText, { color: '#e11d48' }]}>REJECTED</Text>
                </View>
            </View>
            ...
            {item.rejectionReason && (
                <View style={styles.rejectionBox}>
                    <Text allowFontScaling={false} style={styles.rejectionLabel}>REJECTION REASON</Text>
                    <Text allowFontScaling={false} style={styles.rejectionText}>{item.rejectionReason}</Text>
                </View>
            )}

            <View style={styles.photoSectionHeader}>
                <Text style={styles.photoSectionTitle}>REFERENCE ATTACHMENTS</Text>
            </View>
            ...
            <View style={styles.cardFooter}>
                <Text allowFontScaling={false} style={styles.footerDate}>Rejected on: {new Date(item.outTime || item.date).toLocaleDateString()}</Text>
            </View>
        </View>
    );
  };
        ...
        <View style={styles.paddingContainer}>
          <View style={styles.header}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                ...
                <View>
                    <Text allowFontScaling={false} style={styles.headerLabel}>COMPLIANCE ARCHIVE</Text>
                    <Text allowFontScaling={false} style={styles.headerTitle}>Rejected History</Text>
                </View>
            </View>
          </View>
